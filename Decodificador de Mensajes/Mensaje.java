...
public class Mensaje {
    ...
    public void agregarLinea(String linea) {
        ...
        if (linea == "") 
            throw new IllegalArgumentException("La linea a agregar no puede ser vacia");
        lineas.add(linea);
    } 
    ...
    public void agregarLinea(int pos, String linea) { 
        ...
        if (pos < 0 && pos > lineas.size()) 
            throw new IllegalArgumentException("La linea a agregar debe estar ordenada dentro de los valores posibles");
        if (linea == "") 
            throw new IllegalArgumentException("La linea a agregar no puede ser vacia");
    ...
    public boolean equals(Mensaje otro) {
        if (otro == null) 
            throw new IllegalArgumentException("El mensaje a chequear no debe ser null.");
        if (otro.lineas.size() == 0) 
            throw new IllegalArgumentException("El mensaje a chequear esta vacio");      
        boolean sonIguales = false;  
        if (otro.lineas.size() == lineas.size()) {
            int i = 0;
            while (i < otro.lineas.size() && !sonIguales) {
                if (otro.lineas.get(i).equals(lineas.get(i))) {
                    sonIguales = true;
                    i++;
                }
                i++;
