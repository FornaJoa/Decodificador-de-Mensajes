...
public class CodificadorMensajes {
    ...
    private int[] generarCodigoEncripcion(String str) {
        ...
        int sumaChar = 0;
        int sumaTotal = 0;
        String sumaTotalCadena;
        for (int i = 0; i < str.length(); i++) { 
            char caracter = str.charAt(i);
            sumaChar = sumaChar + (int) caracter;
        }
        sumaTotal = sumaChar % 99991;
        sumaTotalCadena = "" + sumaTotal; 
        int[] arrayDigitos = new int[sumaTotalCadena.length()];
        for (int i = 0; i < sumaTotalCadena.length(); i++) { 
            char digitoChar = sumaTotalCadena.charAt(i);
            arrayDigitos[i] = Character.getNumericValue(digitoChar);
        }
        return arrayDigitos;
    }
